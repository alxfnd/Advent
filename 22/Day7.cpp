...
class Directory
{
    ...
    public:
        ...
        string GetParent();
        int output();
        bool DefMe(int realid);
        Directory(string name);
};

bool Directory::DefMe(int realid)
{
    for (int i = 0; i < _subdirs.size(); i++) {
        if (_subdirs[i]->id == realid) {
            return true;
        }
    }
    return false;
...
Directory* SearchDirectories(vector<Directory*>& AllDirectories, string Directory)
{
    for (int i = 0; i < AllDirectories.size(); i++) {
        if (AllDirectories[i]->name == Directory) {
            for (int s = 0; s < AllDirectories.size(); s++) {
                if (AllDirectories[s]->DefMe(AllDirectories[i]->id) == true) {
...
void Processor(string line, Explorer& Explorer, vector<Directory*>& AllDirectories, int count) {
    ...
    switch (a)
    {
        ...
        case 100:
            ...
            cout << "Directory name: " << Explorer.GetDirectory() << endl;
            cout << "Current Dir id is: " << SearchDirectories(AllDirectories, Explorer.GetDirectory())->id << endl;
            cout << "New file is: " << newsubdir->name << endl;
            cout << "Id is: " << newsubdir->id << endl;
            cout << "Directory parent: " << SearchDirectories(AllDirectories, Explorer.GetDirectory())->GetParent() << endl << endl;
            break;
    ...
    if (!(a == 36 || a == 100)) {
        ...
        CD->AddFile(newfile);
        cout << "Directory name: " << Explorer.GetDirectory() << endl;
        cout << "Current Dir id is: " << SearchDirectories(AllDirectories, Explorer.GetDirectory())->id << endl << endl;
